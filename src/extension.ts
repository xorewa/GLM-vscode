...
function readHermesModel(): { model: string; source: 'env' | 'config' | 'fallback' } {
  try {
    const configPath = path.join(os.homedir(), '.hermes-glm', 'config.yaml');
...
function readConfiguredHermesPath(): { value: string; workspaceOverrideIgnored: boolean } {
  const hermesConfig = vscode.workspace.getConfiguration('glm');
...
function resolveHermesBinary(configuredPath: string): string {
  ...
  if (hermesPath !== 'hermes' && !path.isAbsolute(hermesPath)) {
    throw new Error('glm.path must be an absolute path or the default "hermes" value');
  }
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  outputChannel = vscode.window.createOutputChannel('GLM Agent');
  ...
  // GLM fork: run the shared `hermes` agent binary against an isolated home so
  ...
  const glmHome = path.join(os.homedir(), '.hermes-glm');
  client = new AcpClient(
    hermesPath,
    { HERMES_HOME: glmHome, ...(debugLogs ? { HERMES_LOG_LEVEL: 'DEBUG' } : {}) },
    debugLogs,
  );
  ...
  // Commands
  context.subscriptions.push(
    vscode.commands.registerCommand('glm.openChat', async () => {
      outputChannel.appendLine('[ui] open chat');
      await vscode.commands.executeCommand('glm.chatView.focus');
      await ensureConnected();
    }),

    vscode.commands.registerCommand('glm.newSession', () => {
  ...
  statusItem.command = 'glm.openChat';
  statusItem.show();
